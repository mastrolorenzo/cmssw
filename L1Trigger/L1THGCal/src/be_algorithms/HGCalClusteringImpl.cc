...
void HGCalClusteringImpl::clusterise( std::unique_ptr<l1t::HGCalTriggerCellBxCollection>& trgcells_, 
...
//        assert trgcells_->size() > 0 //&& put logWarning
        
    int layer=0;
    for(l1t::HGCalTriggerCellBxCollection::const_iterator tc = trgcells_->begin(); tc != trgcells_->end(); ++tc)
    {

//            if( tc->p4().Pt() > seedThr_ ){
//                tc->setIsSeed();
//            }
